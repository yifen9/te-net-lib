...
def main() -> None:
    args = parse_common_args("Benchmark: te_net_lib.te.lasso_te_matrix")
    ...
    for N, T in sizes:
        ...
        results.append(
            run_case("lasso_te_matrix", params, make_fn, args.warmup, args.repeats)
        )

    print("Benchmark: lasso_te_matrix")
    print(f"repeats={args.repeats} warmup={args.warmup} seed={args.seed}")
    for r in results:
        print(format_row(r.name, r.params, r.stats))

    payload = {
        "bench": "lasso_te_matrix",
        "env": env_info(),
